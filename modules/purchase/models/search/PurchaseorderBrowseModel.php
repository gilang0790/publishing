 ...
 */
class PurchaseorderBrowseModel extends Purchaseorder
{
    public $dateFrom;
    public $dateTo;
    /**
    ...
    public function rules()
    {
        return [
            ...
            [['potransdate', 'potransnum', 'plantid', 'addressbookid', 'paymentmethodid', 'billto', 'shipto', 'headernote', 
    ...
    public function search($params)
    {
        ...
        if (Wfgroup::getMaxStatus('listpo')) {
            $maxStatus = Wfgroup::getMaxStatus('listpo');
        }
        if (Purchaseorderdetail::getOutstanding()) {
            $listOustanding = Purchaseorderdetail::getOutstanding();
        }
        $query->andWhere("ms_company.companyid IN $listCompany");
        $query->andWhere("ms_plant.plantid IN $listPlant");
        $query->andWhere("tr_purchaseorder.status = $maxStatus");
        $query->andWhere("tr_purchaseorder.id IN $listOustanding");
