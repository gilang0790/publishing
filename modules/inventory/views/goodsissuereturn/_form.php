            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <?= $form->field($model, 'girtransdate')->widget(DateControl::className()) ?>
                        </div>
                        <div class="col-md-4">
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <?php DynamicFormWidget::begin([
                        ...
                        'model' => $details[0],
                        'formId' => 'form-goods-issue-return',
                        'formFields' => [
                            'head_id',
                            'productid',
                            'qty',
                        ...
                        <div class="item row">
                            ...
                            <div class="col-sm-12 col-md-6">
                                <?= $form->field($detail, "[{$i}]productid")->widget(Select2::classname(), [
                                    'data' => ArrayHelper::map(ProductSearchModel::findProductIssueReturn($model->goodsissueid)
                            ...
                            <div class="col-sm-4 col-md-2">
                                <?= $form->field($detail, "[{$i}]qty")->textInput(['type' => 'number', 'min' => 1, 'class' => 'form-control text-right girdetailqty']) ?>
                            </div>
...
$(document).ready(function () {
    ...
    $(document).on('change', '#goodsissuereturnform-slocid', function() {
        getStoragebin();
    });
    
    function getStoragebin() {
        var slocid = $('#goodsissuereturnform-slocid').val();
        var i = 0;
        $('.detailProductID').each(function(){
            var storagebinID = 'goodsissuedetailreturn-'+i+'-storagebinid';
            $.ajax({
                type:"POST",
                cache:false,
                url:"lists",
    ...
    $('.item-action').on('click', '.add-item', function (e) {
        ...
        $('#goodsissuereturnform-slocid').val(0).trigger('change');
    });
    ...
    $('.box-footer').on('click', '.btnSave', function (e) {
        ...
        if (!$('#goodsissuereturnform-girtransdate-disp').val()) {
            dateEmptyCounter += 1;
            $('.field-goodsissuereturnform-girtransdate').addClass('has-error')
            alert('Tanggal Transaksi harus diisi!');
            return false;
        }
    
        if (!$('#goodsissuereturnform-slocid').val()) {
            slocEmptyCounter += 1;
            $('.field-goodsissuereturnform-slocid').addClass('has-error')
        ...
        var productID = '';
        $('.detailProductID').each(function(){
            var detailProductID = 'goodsissuedetailreturn-'+i+'-productid';
            var detailQtyID = 'goodsissuedetailreturn-'+i+'-qty';
            var detailStoragebinID = 'goodsissuedetailreturn-'+i+'-storagebinid';
            var detailHppID = 'goodsissuedetailreturn-'+i+'-hpp';
        ...
        if (isSubmit == 1) {
            $('#loading-div').show();
            $('#form-goods-issue-return').submit();
        }
    });
