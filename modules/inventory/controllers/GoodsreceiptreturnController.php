...
use app\modules\inventory\models\forms\GoodsreceiptreturnForm;
use app\modules\inventory\models\Goodsreceiptdetailreturn;
use app\modules\admin\models\Groupmenuauth;
use app\modules\inventory\models\forms\GoodsreceiptreturnApprove;
use app\modules\inventory\models\forms\GoodsreceiptreturnGenerate;
use app\modules\inventory\models\forms\GoodsreceiptreturnReject;
use app\modules\inventory\models\search\GoodsreceiptreturnSearchModel;
use app\modules\inventory\models\search\GoodsreceiptdetailreturnSearchModel;
...
/**
 * GoodsreceiptreturnController implements the CRUD actions for Goods Receipt Return model.
 */
class GoodsreceiptreturnController extends BaseController {
    /**
     * @inheritdoc
     */
    ...
    /**
     * Lists all Goods Receipt Return models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new GoodsreceiptreturnSearchModel();
        $searchModel->dateFrom = date('01-m-Y');
        $searchModel->dateTo = date('d-m-Y');
        $searchModel->grrtransdate = date('d-m-Y');
    ...
    {
        $searchModel = new GoodsreceiptdetailreturnSearchModel();
    ...
    /**
     * Updates an existing Goodsreceiptreturn model.
    ...
    public function actionUpdate($id) {
        ...
        $errMsg = "";
        if (!AppHelper::canAccessData('Goods Receipt Return', $model->grrtransnum)) {
            Yii::$app->session->setFlash('error',
                        'Data #' . $model->grrtransnum . ' sedang diakses oleh ' . User::getUsername($model->lockBy));
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $details = Model::createMultiple(Goodsreceiptdetailreturn::classname(),
                    $details);
            ...
            if ($model->saveModel($errMsg, $details, $deletedIDs)) {
                Yii::$app->session->setFlash('success',
                    'Berhasil simpan retur pembelian' . ' #' . $model->grrtransnum);
            } else {
        ...
        return $this->render('update', [
            'model' => $model,
            'details' => (empty($details)) ? [new Goodsreceiptdetailreturn] : $details
        ]);
    }
    ...
    public function actionGenerate($qq) {
        $model = new GoodsreceiptreturnGenerate();
        $errMsg = "";
        if ($model->generate($errMsg, $qq)) {
            Yii::$app->session->setFlash('success',
                'Berhasil proses data. Silahkan ubah data retur pembelian.');
        } else {
            Yii::$app->session->setFlash('error',
                $errMsg);
        }
    ...
    public function actionApproval($id) {
        ...
        if ($model->approve($errMsg)) {
            Yii::$app->session->setFlash('success',
                'Berhasil proses data' . ' #' . $model->grrtransnum);
        } else {
            Yii::$app->session->setFlash('error',
                $errMsg);
        }
    ...
    public function actionPrint($id) {
        ...
        $pdf->content = $content;
        $pdf->methods = [
            'SetFooter' => ['Retur Pembelian'],
        ];
        return $pdf->render();
    }
        /**
     * Finds the Goodsreceiptreturn model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Goodsreceiptreturn the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = GoodsreceiptreturnForm::findOne($id)) !== null) {
            return $model;
        }
    ...
    protected function findApprovalModel($id) {
        if (($model = GoodsreceiptreturnApprove::findOne($id)) !== null) {
            return $model;
        }
    ...
    protected function findRejectModel($id) {
        if (($model = GoodsreceiptreturnReject::findOne($id)) !== null) {
            return $model;
        }
    ...
    protected function findModelDetail($id) {
        if (($model = Goodsreceiptdetailreturn::find()->where(['head_id' => $id])->all()) !== null) {
            return $model;
        }
