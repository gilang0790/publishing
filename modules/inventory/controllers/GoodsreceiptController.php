...
use app\modules\inventory\models\search\GoodsreceiptSearchModel;
use app\modules\inventory\models\search\GoodsreceiptdetailSearchModel;
use app\modules\inventory\models\Goodsreceiptdetail;
use app\modules\inventory\models\forms\GoodsreceiptForm;
use app\modules\inventory\models\forms\GoodsreceiptApprove;
use app\modules\inventory\models\forms\GoodsreceiptReject;
use app\modules\inventory\models\forms\GoodsreceiptGenerate;
use app\modules\admin\models\Groupmenuauth;
use app\modules\purchase\models\search\PurchaseorderBrowseModel;
...
/**
 * GoodsreceiptController implements the CRUD actions for Goodsreceipt model.
 */
class GoodsreceiptController extends BaseController {
    ...
    /**
     * Lists all Goodsreceipt models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new GoodsreceiptSearchModel();
        $searchModel->dateFrom = date('01-m-Y');
        $searchModel->dateTo = date('d-m-Y');
        $searchModel->grtransdate = date('d-m-Y');
    ...
    {
        $searchModel = new GoodsreceiptdetailSearchModel();
    ...
    public function actionUpdate($id) {
        ...
        $errMsg = "";
        if (!AppHelper::canAccessData('Goods Receipt', $model->grtransnum)) {
            Yii::$app->session->setFlash('error',
                        'Data #' . $model->grtransnum . ' sedang diakses oleh ' . User::getUsername($model->lockBy));
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $details = Model::createMultiple(Goodsreceiptdetail::classname(),
                    $details);
            ...
            if ($model->saveModel($errMsg, $details, $deletedIDs)) {
                Yii::$app->session->setFlash('success',
                    'Berhasil simpan penerimaan barang' . ' #' . $model->grtransnum);
            } else {
        ...
        return $this->render('update', [
            'model' => $model,
            'details' => (empty($details)) ? [new Goodsreceiptdetail] : $details
        ]);
    }
    ...
    public function actionGenerate($qq) {
        $model = new GoodsreceiptGenerate();
        $errMsg = "";
        if ($model->generate($errMsg, $qq)) {
            Yii::$app->session->setFlash('success',
                'Berhasil proses data. Silahkan ubah data penerimaan barang.');
        } else {
            Yii::$app->session->setFlash('error',
                $errMsg);
        }
    ...
    public function actionApproval($id) {
        ...
        if ($model->approve($errMsg)) {
            Yii::$app->session->setFlash('success',
                'Berhasil proses data' . ' #' . $model->grtransnum);
        } else {
            Yii::$app->session->setFlash('error',
                $errMsg);
        }
    ...
    public function actionBrowse() {
        $searchModel = new PurchaseorderBrowseModel();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
    ...
    public function actionPrint($id) {
        ...
        $pdf->content = $content;
        $pdf->methods = [
            'SetFooter' => ['Penerimaan Barang'],
        ];
        return $pdf->render();
    }

    /**
     * Finds the Goodsreceipt model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Goodsreceipt the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = GoodsreceiptForm::findOne($id)) !== null) {
            return $model;
        }
    ...
    protected function findApprovalModel($id) {
        if (($model = GoodsreceiptApprove::findOne($id)) !== null) {
            return $model;
        }
    ...
    protected function findRejectModel($id) {
        if (($model = GoodsreceiptReject::findOne($id)) !== null) {
            return $model;
        }
