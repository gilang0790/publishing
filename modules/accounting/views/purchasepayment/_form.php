            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <?= $form->field($model, 'pptransdate')->widget(DateControl::className()) ?>
                        </div>
                        <div class="col-md-2">
                        ...
                        <div class="col-md-2">
                            <?= $form->field($model, 'invoiceapid')->widget(Select2::classname(), [
                                'data' => ArrayHelper::map(InvoiceapBrowseModel::findActive()->orderBy('id')->all(),
                                        'id', 'aptransnum'),
                                'options' => [
                                    'prompt' => '--- Pilih Faktur Pembelian ---'
                                ],
                                'pluginOptions' => [
                                    'allowClear' => true
                                ],
...
$(document).ready(function () {
    ...
    $('.box-footer').on('click', '.btnSave', function (e) {
        ...
        var strApamount = $('#purchasepaymentform-apamount').val();
        var strPayamount = $('#purchasepaymentform-stringpayamount').val();
        var apamount = strApamount.split('.').join("");
        var payamount = strPayamount.split('.').join("");
        if (payamount > apamount) {
            alert('Pembayaran tidak boleh melebihi sisa tagihan.');
            return false;
        } else {
            $('#loading-div').show();
            $('#form-purchase-payment').submit();
        }
